...
public class HttpClient {
	...
	public static boolean isNetworkConnected(Context ctx) {
		...
		if (conMan != null) {
			...
			/********
			 * ＜クラッシュレポートにより以下のコードは削除＞ // 3G回線の状態 State mobile =
			 * conMan.getNetworkInfo
			 * (ConnectivityManager.TYPE_MOBILE).getState(); // WIFIの状態 State
			 * wifi =
			 * conMan.getNetworkInfo(ConnectivityManager.TYPE_WIFI).getState();
			 * 
			 * // いずれかが接続されているか？ if((mobile != State.CONNECTED) && (wifi !=
			 * State.CONNECTED)) { return false; } // どちらか、または両方接続されている return
			 * true;
			 ********/
		}
		return false;
	...
	/**
	 * 
	 * 引数で指定されるURLのデータをバイト配列で取得する。 WEBアクセスによるXMLデータの取得や画像データの取得などによる利用を想定している。
	 * 
	 * @param strUrl
	 *            WEBアクセスによるデータ取得のURL
