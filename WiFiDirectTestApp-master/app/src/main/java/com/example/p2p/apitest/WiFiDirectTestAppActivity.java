...
public class WiFiDirectTestAppActivity extends Activity {
    ...
    public void onClickRequestGroupInfo(View view) {
        ...
        mWifiP2pManager.requestGroupInfo(mChannel, new GroupInfoListener() {
            ...
            public void onGroupInfoAvailable(WifiP2pGroup group) {
                ...
                if (HTML_OUT)
                    pass = "<font color=red><b>" + pass + "</b></font>"; // たぶんfont colorのネストはできない(パスワードが赤にならない)
                log += pass;
    ...
    /**
     * created by fukuda
     * パスワードを取得してSSIDの末尾に付与
     *
     * @param view
     */
    public void onClickSetNetworkName(View view) {
        if (isNull(true)) {
            return;
        }
        mWifiP2pManager.requestGroupInfo(mChannel, new GroupInfoListener() {
...
//                String mSsidName = group.getNetworkName();
//                mSsidName += group.getPassphrase();

                // サンプルSSIDを設定
//                String mSsidName = "DRECT- :" + group.getPassphrase();
                String mSsidName = "DRECT- : パスワード";

                try {
                    // リフレクション
                    Class c;
                    c = Class.forName("android.net.wifi.p2p.WifiP2pGroup");
                    Method[] methods = c.getDeclaredMethods();
//                    Method m = c.getMethod("setNetworkName", new Class[] {String.class});
                    for (Method m : methods) {
                        try {
                            if (m.getName().equals("setNetworkName")) {
                                // 名前が一致すれば実行
                                m.invoke(group, new Object[]{mSsidName});
                                break;
                            }
                    ...
                    Log.d("group.getNetworkName():", group.getNetworkName());
                    addLog("group.getNetworkName():" + group.getNetworkName() + nl());
                } catch (ClassNotFoundException e) {
                    // classが見つからないとき
                } catch (Exception e) {
                }

                addLog(group.toString() + nl());
            }
        });
    }

    public void onClickSetPassphrase(View view){
        if (isNull(true)) {
            return;
        }
        mWifiP2pManager.requestGroupInfo(mChannel, new GroupInfoListener() {
            ...
            public void onGroupInfoAvailable(WifiP2pGroup group) {
                ...
                try {
                    ...
                    for (Method m : methods) {
                        try {
                            if (m.getName().equals("setPassphrase")) {
                                // 名前が一致すれば実行
                                m.invoke(group, new Object[]{mPassword});
                                break;
                            }
                        } catch (IllegalAccessException e1) {
                            // 呼び出し：アクセス違反、保護されている
                    ...
                    Log.d("group.getPassphrase():", group.getPassphrase());
                    addLog("group.getPassphrase():" + group.getPassphrase() + nl());
                } catch (ClassNotFoundException e) {
                    // classが見つからないとき
                } catch (Exception e) {
                }
